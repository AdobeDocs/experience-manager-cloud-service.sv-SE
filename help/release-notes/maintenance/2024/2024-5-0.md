...
* Vissa användare kan ha problem med att skapa formulär på `Cloud Service version 2024.5.16461`när en användare skapar ett formulär visas följande felmeddelande i dialogrutan för att skapa:

  `A server error occurred. Try again after sometime.`

  Problemet inträffar eftersom författaren publicerar formuläret utan att först publicera mallen som används i formuläret, vilket resulterar i att `jcr:uuid` och andra skyddade och systemgenererade egenskaper för `<template-path>/initial/jcr:content` nod. Detta orsakar fel vid efterföljande formulärgenerering.

  Lös problemet genom att följa stegen nedan:

   1. Kontrollera att mallen som du använder i formuläret inte har `jcr:uuid` och andra systemgenererade skyddade egenskaper på sökvägen `<template-path>/initial/jcr:content` nod.
   ...
   1. Om mallen som du använde uppdaterar i framtida versioner publicerar du mallen igen (som i steg 2) för att förhindra att det uppstår problem när du skapar formulär.
