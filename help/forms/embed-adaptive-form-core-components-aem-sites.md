...
* **Skapa ett adaptivt formulär med komponenten Adaptiv Forms-behållare**: Med komponenten [Adaptiv formulärbehållare](#af-container-component) kan du skapa digitala registreringsupplevelser genom att använda adaptiva Forms-komponenter direkt i AEM Sites-redigeraren. Integreringen ger en smidig upplevelse för de som skapar AEM Sites och vill skapa och hantera formulär på sina AEM Sites-sidor.

* **Lägg till ett befintligt adaptivt formulär**: Med komponenten [Adaptiv Forms - Embed(v2)](#embed-existing-af) kan du enkelt lägga till ett befintligt adaptivt formulär på en sida i AEM Sites. Den här funktionen gör det enklare att anpassa och återanvända Adaptive Forms. Den här integreringen är ett bekvämt sätt för kunder att återanvända adaptiva Forms som de redan har skapat.

* **Använd guiden Adaptiv Forms för att skapa ett formulär**: Använd komponenten [Adaptiv Forms - Embed(v2)](#embed-new-af) för att skapa ett adaptivt formulär i AEM Sites-redigeraren med hjälp av guiden Skapa formulär. Formuläret sparas som en extern entitet. Du kan även återanvända det här formuläret på andra webbplatssidor och i fristående formulär.

* **Lägg till flera adaptiva Forms på en AEM Sites-sida**: Om du vill lägga till flera adaptiva Forms på en AEM Sites-sida använder du AEM Forms behållarkomponenter - [Adaptiv Forms - Embed(v2)](#embed-new-af) och [Adaptiv formulärbehållare](#af-container-component). Om du behöver lägga till mer än ett adaptivt formulär som en div på en AEM Sites-sida kan du använda komponenten Adaptiv formulärbehållare.
...
Komponenten [!UICONTROL Adaptive Form Container] gör det möjligt att skapa digitala registreringsupplevelser med hjälp av adaptiva Forms-komponenter i AEM Sites Editor. Du kan skapa ett anpassat formulär genom att dra och släppa formulärkomponenterna.

### Förutsättningar {#prerequisites-af-container}

+++ Aktivera komponenten **[!UICONTROL Adaptive Forms Container]**.
...
1. Klicka på [!UICONTROL Policy] och markera kryssrutan **[!UICONTROL Adaptive Forms Container]** under **[AEM Archetype Project Name] - Adaptive Form**.
...
1. Kopiera värdet för `sling:resourceSuperType`. Värdet är till exempel `core/wcm/components/page/v3/page`.
   ...
   ![avlutningsresurs](/help/forms/assets/slingresource.png)
...
Så här skapar du ett adaptivt formulär med komponenten [!UICONTROL Adaptive Forms Container]:

1. Öppna AEM Sites-sidan i redigeringsläge.
1. Dra och släpp komponenten **[!UICONTROL Adaptive Forms Container]** på sidan från panelen Komponentwebbläsaren.
...
Du kan anpassa de avancerade inställningarna för komponenten [!UICONTROL Adaptive Form Container]. Exempel:
...
Om du vill ange egenskaper för komponenten **[!UICONTROL Adaptive Forms Container]** klickar du på ![ settings_icon ](/help/forms/assets/Smock_Wrench_18_N.svg) i åtgärdsfältet. Dialogrutan **[!UICONTROL Edit Adaptive Forms Container]** öppnas.

![Dialogrutan Redigera](/help/forms/assets/adaptiveformcontainer-editdialog.png)

I dialogrutan [!UICONTROL Edit Adaptive Forms Container] kan du ange följande:
* **Grundläggande flik**
   * **Förifyll tjänst**: Du kan använda förifyllningstjänsten för att autofylla fält i ett adaptivt formulär med befintliga data. När en användare öppnar ett formulär är värdena för dessa fält förifyllda. Mer information om förifyllningstjänsten finns i [Förifyll adaptiva formulärfält](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-foundation-components/prepopulate-adaptive-form-fields.html#configuring-prefill-service-using-configuration-manager)
   * **Klientbibliotekskategori**: Ange de [JavaScript-funktioner](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-foundation-components/add-rules-and-use-expressions-in-an-adaptive-form/rule-editor.html?lang=en#custom-functions) som används i uttryck och stöds av Adaptiv Forms.
* **Datamodell**: Med en datamodell kan du integrera entiteter och tjänster från olika datakällor i ett adaptivt formulär. Välj **[!UICONTROL Form Data Model]** om det adaptiva formulär som du skapar inbegriper att hämta och skriva data från och till flera datakällor.
   ...
   * **Schema**: Schemat representerar strukturen i vilken data produceras eller används av det bakomliggande systemet i din organisation. Du kan [koppla schemat till ett adaptivt formulär](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-foundation-components/create-an-adaptive-form-on-forms-cs/adaptive-form-json-schema-form-model.html) och använda dess element för att lägga till dynamiskt innehåll i ett adaptivt formulär.
...
* **fliken Skicka**
   ...
   * **Omdirigera till URL**
      ...
      * **Skicka åtgärd**: En skicka-åtgärd utlöses när en användare klickar på knappen Skicka i ett anpassat formulär. Du kan [konfigurera skicka-åtgärden för anpassat formulär](/help/forms/configuring-submit-actions.md). Adaptiva formulär innehåller följande inskickningsåtgärder:
  ...
  Du kan också [utöka standardåtgärden för att skicka ](custom-submit-action-form.md) och skapa en egen anpassad åtgärd för att skicka.
...
Med komponenten **[!UICONTROL Adaptive Forms - Embed (V2)]** kan du antingen bädda in ett nytt adaptivt formulär eller bädda in ett befintligt adaptivt formulär på webbplatsens sida. Med komponenten [!UICONTROL Adaptive Forms - Embed(v2)] kan du:
...
+++ Aktivera komponenten **Adaptiv Forms - Bädda in**.

Så här aktiverar du komponenten [!UICONTROL Adaptive Forms - Embed(v2)] i mallprincipen:

1. Gå till [!UICONTROL Page Information] > [!UICONTROL Edit Template]

1. Klicka på [!UICONTROL Policy] och markera kryssrutan **[!UICONTROL Adaptive Form - Embed (v2)]** under gruppen **[!UICONTROL [AEM Archetype Project Name] - Forms]** .
1. Klicka på **[!UICONTROL Done]**.
...
När alternativet **[!UICONTROL When form covers entire width of a page]** är markerat i dialogrutan **[!UICONTROL Form Containers]** Konfigurera och **Adaptiv Forms med kärnkomponenter** används, måste du ta med klientlibs på sidan för motsvarande webbplats.

![Övertäckning GIF](/help/forms/assets/overlaycorecomponent.gif)

Om du vill använda adaptiva Forms-komponenter på en AEM Sites-sida inkluderar du klientbiblioteken `Customheaderlibs` och `Customfooterlibs` på AEM Sites-sidan med hjälp av AEM Archetype/Git Repository och distributionsflödet.
...
1. Öppna AEM Sites-sidan i redigeringsläge.
1. Dra och släpp komponenten [!UICONTROL Adaptive Forms - Embed] på sidan från panelen Komponentwebbläsaren.
1. Markera [!UICONTROL Adaptive Forms - Embed]-komponenten på webbplatssidan och välj ![ settings_icon ](/help/forms/assets/Smock_Wrench_18_N.svg) i åtgärdsfältet. Dialogrutan **[!UICONTROL Edit Adaptive Forms - Embed]** öppnas.
...
1. [!UICONTROL Asset Path] innehåller redan sökvägen till ett skapat anpassat formulär
1. Spara inställningarna. Det adaptiva formuläret är nu inbäddat på sidan.
...
Du kan anpassa de avancerade inställningarna för komponenten [!UICONTROL Adaptive Form - Embed(v2)]. I dialogrutan [!UICONTROL Edit Adaptive Forms - Embed(v2)] kan du ange följande:

* **Resurssökväg**: Bläddra och välj det anpassade formulär som ska bäddas in. Den fylls i automatiskt om du utelämnade den från Assets webbläsare.
...
### Publish lade till Adaptiv Forms med hjälp av adaptiv form - komponenten Embed(v2)  {#publish-embedded-adaptive-form}

Titta på följande scenarier för publicering av tillagd Adaptive Forms med komponenten **[!UICONTROL Adaptive Form - Embed(v2)]**:
...
>* [Bädda in anpassat formulär på en extern webbsida](/help/forms/embed-adaptive-form-external-web-page.md)