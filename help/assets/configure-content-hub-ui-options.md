...
1. Ange en etikett för egenskapen, mappa den till en egenskap med hjälp av fältet **[!UICONTROL Metadata]** och välj lämplig indatatyp för de nya metadata för resursen. Indatatypsetiketterna uppdateras för att anpassas till Adobe Experience Manager (AEM). De tillgängliga indatatyperna är Text, Taggar, Datum, Nummer, Listruta, Flervärdestext, Smarta taggar och Smarta färgtaggar. För vissa metadatafält (xcm:colorDistribution, xcm:machineKeywords) tilldelas indatatypen automatiskt och kan inte ändras.
...
![Överföringsinformation för konfigurationsgränssnittet på Content Hub](assets/filter-new.png)
...
1. Ange en etikett för filtret, mappa det till en egenskap med fältet **[!UICONTROL Metadata]** och välj lämplig indatatyp för det nya filtret. Indatatypsetiketterna uppdateras för att anpassas till Adobe Experience Manager (AEM). De tillgängliga indatatyperna är Text, Taggar, Datum, Nummer, Listruta, Flervärdestext, Smarta taggar och Smarta färgtaggar. För vissa metadatafält (xcm:colorDistribution, xcm:machineKeywords) tilldelas indatatypen automatiskt och kan inte ändras.
...
![Gränssnittsfilter för konfiguration på Content Hub](assets/add-filters.png)
...
1. Aktivera växlingen **[!UICONTROL Bulk Search]**. Standardavgränsarna `[ , | \t | \r\n | \r | \n ]` visas automatiskt. Dessutom kan du lägga till ytterligare avgränsare. Om du vill göra det anger du avgränsare i indatarutan avgränsade med `pipe symbol (|)`.
   ...
   ![Konfiguration för masssökning](assets/edit-filter-new.png)
...
![Information om användargränssnittsresurser för konfiguration på Content Hub](assets/asset-detailsn.png)
...
### Vem har åtkomst till och kan konfigurera inställningarna för användargränssnittet i AEM Assets Content Hub?

Endast användare med Content Hub administratörsbehörighet kan komma åt och konfigurera inställningarna för Content Hub användargränssnitt. Administrativ behörighet måste beviljas via Adobe Admin Console innan konfigurationsgränssnittet kan nås.

### Vad kan administratörer konfigurera i användargränssnittet för AEM Assets Content Hub?

Administratörer kan konfigurera följande inställning i AEM Assets Content Hub Configuration User Interface:

* Importera metadatafält (inklusive obligatoriska fält och inställningar för automatiskt godkännande)

* Sökfilter som visas på söksidan

* Sökbara metadatafält för indexering

* Resurskortsmetadata (upp till sex fält visas på kort)

* Vyegenskaper för tillgångsinformation

* Varumärkeselement (logotyp, banderolltext, färger, banderollbild)

* Regler för synlighet av tillgångar (inklusive beteenden för utgånna tillgångar)

* Återgivnings- och nedladdningsalternativ

* Anpassade navigeringslänkar (externa flikar)

* Inställningar för insamling och delning (offentliga länkar och behörigheter)

De här inställningarna styr både visuell upplevelse och funktionalitet i Content Hub för slutanvändare.


### Hur lägger administratörer till nya metadatafält för överföringar av resurser i AEM Assets Content Hub?

Administratörer kan lägga till nya metadatafält för överföringar av resurser genom att klicka på **Lägg till metadata** på fliken **Importera**, ange etiketten, mappa den till lämplig egenskap från metadataschemat, välja indatatyp och avgöra om fältet är obligatoriskt eller valfritt. När du har bekräftat och sparat blir det nya fältet tillgängligt för användare vid överföring av resurser.

### Vad är inställningen för automatiskt godkännande för resurser i AEM Assets Content Hub och hur fungerar den?

När inställningen för automatiskt godkännande är aktiverad blir nya resurser omedelbart tillgängliga på Content Hub vid överföring. Som standard är den här inställningen inaktiverad, vilket kräver att AEM författare eller administratörer manuellt godkänner nya resurser innan de visas på Content Hub-portalen.

### Hur konfigurerar administratörer filter för resurssökningar i AEM Assets Content Hub?

Administratörer kan konfigurera filter för resurssökningar genom att klicka på **Lägg till filter**, ange filteretiketten, mappa den till rätt metadataegenskap, välja datatyp och bekräfta tillägget. Filter blir tillgängliga i gränssnittet när det finns minst en resurs som matchar filtervillkoren. Ändringarna tillämpas genom att klicka på **Spara**.

### Vilka resursegenskaper kan visas i vyn Resursinformation och hur konfigureras de i AEM Assets Content Hub?

Resursegenskaper som filnamn, titel, format, storlek och anpassade fält som kategorier kan visas i vyn Resursinformation. Administratörer lägger till de här egenskaperna genom att klicka på **Lägg till metadata**, ange etiketten, mappa den till metadataschemat, välja datatyp, bekräfta och spara ändringarna.

### Hur kan administratörer anpassa metadatainformation som visas på resurskort i AEM Assets Content Hub?

Administratörer kan anpassa metadatainformation som visas på tillgångskort genom att klicka på **Lägg till metadata** i resurskortsinställningarna, ange etiketten (t.ex. filformat), mappa den till lämplig egenskap i metadataschemat och bekräfta tillägget. Upp till sex metadatafält kan visas på resurskort.

### Kan administratörer anpassa din AEM Assets Content Hub-portal efter företagets grafiska profil?

Ja. Administratörer kan anpassa utseendet och känslan i AEM Assets Content Hub genom att redigera rubriktexten och brödtexten för huvudbanderollen, välja textfärger via en färgväljare eller hexadecimal kod samt ange sekundära och primära färger för bakgrund, text och ikoner.
