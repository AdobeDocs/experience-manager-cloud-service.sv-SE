...
Alla ändringar av metadata för en resurs skrivs tillbaka till den ursprungliga binärfilen som en del av dess XMP-data. Detta görs via Experience Manager metadata write-back. Ändringar som görs i befintliga egenskaper (till exempel `dc:title`) skrivs över och skapade egenskaper (inklusive anpassade egenskaper som `cq:tags`) läggs ihop med schemat.
