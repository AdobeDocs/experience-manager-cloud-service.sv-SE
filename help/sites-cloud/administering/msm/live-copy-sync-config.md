...
| Skjut vid ändring | Flyttar innehållet till Live Copy när källan ändras<br>Använd den här utrullningskonfigurationen sparsamt när den använder utlösaren Vid ändring. | Vid ändring | `contentUpdate`<br>`contentCopy`<br>`contentDelete`<br>`referencesUpdate`<br>`orderChildren` |
