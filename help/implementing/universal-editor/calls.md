...
* `itemid`: JCR-sökvägen som ska uppdateras
* `itemprop`: Den JCR-egenskap som uppdateras
* `itemtype`: JCR-värdetypen för egenskapen som uppdateras
...
A `patch` inträffar när du uppdaterar innehåll i en dialogruta i appen. Detta uppdaterar innehållet på appsidan som en JSON-korrigering till det befintliga innehållet.

Dess nyttolast inkluderar sökvägen för innehållet på sidan samt JSON-korrigeringen för den ändring som ska göras.

### Exempel på nyttolast {#patch-payload}
