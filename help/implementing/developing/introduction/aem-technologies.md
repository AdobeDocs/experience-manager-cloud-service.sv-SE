...
Java Content Repository (JCR)-standarden, [JSR 283](https://docs.adobe.com/content/docs/en/spec/jcr/2.0/index.html), anger ett leverantörsoberoende och implementeringsoberoende sätt att få åtkomst till innehåll dubbelriktat på en detaljnivå i en innehållsdatabas. Specifikationsledaren innehas av Adobe Research (Schweiz) AG.

Paketet [JCR API 2.0](https://docs.adobe.com/docs/en/spec/javax.jcr/javadocs/jcr-2.0/index.html) , `javax.jcr.*` används för direkt åtkomst och redigering av databasinnehåll.

AEM bygger på en JCR.

## Apache Jackrabbit Oak {#jackrabbit-oak}

[Apache Jackrabbit Oak](https://jackrabbit.apache.org/oak/) är en implementering av en skalbar och högpresterande hierarkisk innehållsdatabas som kan användas som grund för moderna webbplatser i världsklass och andra krävande innehållsprogram, som uppfyller JCR-standarden.
...
Alla Sling-skript lagras i undermappar till antingen `/apps` (mutable, user scripts) eller `/libs` (immutable, system scripts), som söks igenom i den här ordningen.
...
Om föregående exempel `sling:resourceType` är `hr/jobs` för:

* GET/HEAD och URL:er som slutar i `.html` (standardbegärandetyper, standardformat)
   * Skriptet ska vara `/apps/hr/jobs/jobs.esp`; det sista avsnittet i `sling:resourceType` formulären filnamnet.
...
* Om inget `sling:resourceType` har definierats:
...
* Om inget skript hittas används standardskriptet.
   * Standardåtergivningen stöds för närvarande som oformaterad text (`.txt`), HTML (`.html`) och JSON (`.json`), som alla listar nodens egenskaper (lämpligt formaterade). Standardåtergivningen för tillägget `.res`, eller begäranden utan ett tillägg, är att resursen (där det är möjligt) ska tas bort.
...
Detta beror på att `/y` har `sling:resourceSuperType` egenskapen men `/x` inte, och dess supertyp tas därför från sin resurstyp.
...
Om du vill göra det kan du använda `sling:include("/<path>/<resource>")` kommandot. Detta inkluderar effektivt definitionen av den refererade resursen.
...
Med den här arkitekturen kan du utöka Sling med programspecifika moduler. Sling, och därmed AEM, använder [Apache Felix](https://felix.apache.org/) -implementeringen av OSGi. De är båda samlingar av OSGi-paket som körs i ett OSGi-ramverk.
...
* `/libs` - Bibliotek och definitioner som hör till kärnan i AEM. Undermapparna i `/libs` representerar de färdiga AEM. Innehållet i `/libs` kan inte ändras. Funktioner som är specifika för din webbplats bör göras under `/apps`.
