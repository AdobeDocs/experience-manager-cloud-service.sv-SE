...
# Använda bibliotek på klientsidan AEM som en Cloud Service {#using-client-side-libraries}

Digitala upplevelser är till stor del beroende av bearbetning på klientsidan som styrs av komplex JavaScript- och CSS-kod. Med AEM-bibliotek (klientbibliotek) kan du ordna och centralt lagra dessa klientbibliotek i databasen. I kombination med [utvecklingsprocessen i den AEM projekttypen blir det](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend.html) enkelt att hantera koden i AEM.
...
## Front-End-utveckling för AEM som Cloud Service {#fed-for-aemaacs}
...
Arkitypen kan sedan kompilera resurserna till en enda CSS- och JS-fil, och bädda in dem automatiskt i en `cq:clientLibraryFolder` databas.
...
* `cq:ClientLibraryFolder` noder kan placeras var som helst i databasens `/apps` underträd.
* Använd nodens `categories` egenskap för att identifiera de bibliotekskategorier som den tillhör.
...
* `allowProxy`: Eftersom alla klientbibliotek måste lagras under `apps`kan den här egenskapen ge åtkomst till klientbibliotek via en proxyserver. Se [Hitta en biblioteksmapp för klient och Använda servern för](#locating-a-client-library-folder-and-using-the-proxy-client-libraries-servlet) proxyklientbibliotek nedan.
...
Om klientbiblioteksmappen innehåller en eller flera källfiler som sammanfogas till en enda JS- och/eller CSS-fil vid körning. Den genererade filens namn är nodnamnet med filnamnstillägget `.js` eller `.css` . Till exempel resulterar biblioteksnoden med namnet `cq.jquery` i den genererade filen med namnet `cq.jquery.js` eller `cq.jquery.css`.
...
1. Markera `/apps` mappen och klicka på **Skapa > Skapa nod**.
...
1. Om du vill ange kategorin eller kategorierna som biblioteket tillhör markerar du `cq:ClientLibraryFolder` noden, lägger till följande egenskap och klickar sedan på **Spara alla**:
   ...
   * Flera: Markerad
1. Om du vill att klientbiblioteken ska vara tillgängliga via proxy under `/etc.clientlibs`markerar du `cq:ClientLibraryFolder` noden, lägger till följande egenskap och klickar sedan på **Spara alla**:
   * Namn: `allowProxy`
   * Typ: Boolean
   * Värde: `true`
...
1. Lägg till källfiler i biblioteksmappen.
   * Detta görs vanligtvis i den första byggprocessen av [AEM Project Archetype.](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend.html)
...
När klientbiblioteksmappen är [konfigurerad som](#creating-clientlib-folders) du vill kan du begära klientlibs via proxy. Exempel:
...
Med `allowProxy` egenskapen kan du begära:
...
Klientbibliotek läses in via en hjälpmall från AEM, som du kommer åt via `data-sly-use`. Hjälpmallar är tillgängliga i den här filen, som kan anropas `data-sly-call`.
...
[Mer information om hur du läser in klientlibs via HTML finns i dokumentationen](https://docs.adobe.com/content/help/en/experience-manager-htl/using/getting-started/getting-started.html#loading-client-libraries) till HTML.
...
De flesta klientlibs krävs i den AEM publiceringsinstansen. Det vill säga att de flesta kundens syften är att skapa en användarupplevelse av innehållet. Klientlibs för publiceringsinstanser kan användas och distribueras med [gränssnittsverktygen](#fed-for-aemaacs) i [klientbiblioteksmapparna enligt beskrivningen ovan.](#creating-clientlib-folders)
...
Om du behöver använda klientbibliotek på författaren kan du skapa dina klientbibliotek på samma sätt `/apps` som för publicering, men skriva det direkt under `/apps/.../clientlibs/foo` istället för att skapa ett helt projekt för att hantera det.
...
Ytterligare inställningar för klientbibliotek kan styras via panelen **Adobe Granite HTML Library Manager** i System Console på `https://<host>:<port>/system/console/configMgr`).
...
* `embed`: Används för att bädda in kod från andra bibliotek. Om noden `F` bäddar in noder `G` och `H`HTML blir resultatet en sammanfogning av innehåll från noder `G` och `H`.
...
Exempelvis `/etc/clientlibs/myclientlibs/publicmain` är biblioteket beroende av `cq.jquery` biblioteket. Sidan som refererar till huvudklientbiblioteket genererar HTML som innehåller följande kod:
...
I vissa fall kan det hända att den slutliga HTML-koden som genereras för den typiska sidan av publiceringsinstansen innehåller ett relativt stort antal `<script>` element.
...
I exemplet i föregående [inbäddningskod från andra bibliotek](#embedding-code-from-other-libraries) bäddar `/etc/client/libraries/myclientlibs/publicmain` klientbiblioteksmappen in `/apps/myapp/clientlib` klientbiblioteksmappen. Om du lägger till parametern på webbsidan skapas följande länk i webbsidans källkod:
