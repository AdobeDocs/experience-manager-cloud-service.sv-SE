...
description: Lär dig rekommendationer om hur du skapar ett projekt på ett smidigt sätt med lokaliserade webbplatser som utnyttjar en enda kodbas, som alla kan hanteras av Edge Delivery Services.
...
Lär dig rekommendationer om hur du skapar ett projekt på ett smidigt sätt med lokaliserade webbplatser som utnyttjar en enda kodbas, som alla kan hanteras av Edge Delivery Services.
...
Innehåll i `language-masters` är källan till Live-kopior för de lokaliserade webbplatserna: Tyskland (`de`) och Schweiz (`ch`). Målet med det här dokumentet är att skapa Edge Delivery Services som alla använder samma kodbas för varje lokaliserad plats.
...
### Skapa nya Edge Delivery Services för lokaliserade sidor {#create-edge-sites}

Om du vill ansluta fler webbplatser till Edge Delivery Services för en flerspråkig platskonfiguration för flera regioner måste du skapa en ny aem.live-webbplats för var och en av dina AEM MSM-webbplatser. Det finns en 1:1-relation mellan AEM MSM-webbplatser och aem.live-webbplatser med en delad Git-databas och kodbas.

I det här exemplet skapar vi platsen `wknd-ch` för den schweiziska närvaron av wknd, vars lokaliserade innehåll finns under AEM `/content/wknd/ch`.
...
1. Logga in på AEM författarinstans och gå till **Verktyg** -> **Cloud Service** -> **Konfiguration av Edge Delivery Services**.
...
1. Besök webbplatsen för nya Edge Delivery Services för den lokaliserade sidan på `https://main--wknd-ch--<your-github-org>.aem.page`.
