---
title: Översikt över Experience Modernization Agent
description: Läs om hur Experience Modernization Agent tar in nya webbplatser i Edge Delivery Services med hjälp av AI.
...
# Översikt över Experience Modernization Agent {#experience-modernization-agent}

Läs om hur Experience Modernization Agent tar in webbplatser i Edge Delivery Services med hjälp av AI.
...
Experience Modernization Agent utnyttjar Edge Delivery Services till fullo (inklusive AEM) genom att göra webbplatsmigreringar och den pågående utvecklingen snabb och smidig.
...
## Funktioner för blockutveckling {#block-development}

Adobe Experience Modernization Agent utnyttjar de allmänna utvecklingsfunktionerna i Edge Delivery Services som kan användas för olika utvecklingsuppgifter och ger ett kontinuerligt värde utöver att skapa eller migrera webbplatser.

* Använder innehållsstyrd utvecklingsmetod (CDD) för designvänliga innehållsmodeller
* Utnyttjar [blocksamlingen](https://www.aem.live/developer/block-collection) och [blockgruppen](https://www.aem.live/developer/block-party/) för att hitta referensimplementeringar och bästa praxis
* Stöder arbetsflöden för testning och felsökning för validering av ändringar före driftsättning

Detaljerade funktioner omfattar blockutveckling, innehållsmodellering, identifiering av referensblock, testning och felsökning.
...
* Komplext dynamiskt innehåll, t.ex. innehåll som kräver avancerad användarinteraktion, visas i DOM.
   * Återgivet innehåll på klientsidan stöds om innehållet är tillgängligt via en viss URL.
   * Element som är dolda via CSS men som finns i DOM, som flikar, dragspel eller karuseller, stöds också.

Agenten stöder inte följande mål.

* AEM publiceringsmiljöer där webbplatser använder HTML-baserad leverans
...
* Integreringar av data/tjänster från tredje part på server- eller klientsidan
* Integrering av e-handel eller sökfunktioner
* MarTech datalager eller målinriktning/experiment
* Isolering av innehålls-/upplevelsefragment
